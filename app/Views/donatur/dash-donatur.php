...
$myActiveTransactions = array_values(array_filter($myDonations, fn($d) => ($d['status'] ?? '') !== 'rejected'));


$totalDonation = 0;
$supportedPrograms = [];
foreach ($myDonations as $donation) {
    if (($donation['status'] ?? '') !== 'verified') continue;
...
// Total dana terkumpul dihitung dari donasi verified (bukan sum field collected program)
$totalCollected = 0;
foreach ($_SESSION['donations'] ?? [] as $dv) {
    if (($dv['status'] ?? '') === 'verified') {
        $totalCollected += (int) str_replace(['.', ','], ['', ''], $dv['amount'] ?? '0');
    }
}
// Konversi ke satuan juta untuk tampilan (konsisten dengan formatJuta)
$totalCollectedJuta = round($totalCollected / 1000000, 1);
$totalDonors = count(array_unique(array_map(
    fn($d) => $d['donor'],
    array_filter($_SESSION['donations'], fn($d) => ($d['status'] ?? '') === 'verified')
)));
...
<div class="donor-landing" id="donorLanding">
    <section class="dl-hero">
        ...
        <div class="dl-hero-stats">
            ...
            <div class="dl-hero-stat">
                <strong><?= e(number_format($totalCollectedJuta, 1, ',', '.')) ?>Jt</strong>
            ...
            <div class="dl-program-grid" id="programGrid">
                <?php foreach ($programs as $idx => $p): ?>
                    ...
                    <a class="dl-program-card" href="index.php?route=app&page=program-detail&id=<?= e($p['id']) ?>" data-title="<?= e(strtolower($p['name'])) ?>" data-category="<?= e($p['cat']) ?>">
                        <?php $cardImage = sipedo_program_image($p); ?>
                        <div class="dl-card-image" style="background: <?= $cardImage !== '' ? 'linear-gradient(180deg, rgba(15,31,61,.12), rgba(15,31,61,.18)), url(\'' . e(pub($cardImage)) . '\') center/cover no-repeat' : e(program_gradient_dashboard($idx)) ?>;">
                            <?php if ($cardImage === ''): ?>
                        ...
                        <div class="dl-card-body">
                            <h3><?= e($p['name']) ?></h3>
                            <p class="dl-card-desc"><?php
                                $rawD = trim($p['desc'] ?? '');
                                if ($rawD === '') $rawD = 'Program ' . strtolower($p['cat']) . ' dengan deadline ' . $p['deadline'] . '. Bantu program ini mencapai target donasi.';
                                // Ambil baris pertama yang tidak kosong
                                foreach (preg_split('/\r?\n/', $rawD) as $ln) {
                                    if (trim($ln) !== '') { $rawD = trim($ln); break; }
                                }
                                // Strip emoji & potong max 75 karakter
                                $rawD = preg_replace('/[\x{1F000}-\x{1FFFF}\x{2600}-\x{27FF}\x{FE00}-\x{FE0F}]/u', '', $rawD);
                                $rawD = trim($rawD);
                                echo e(mb_strlen($rawD) > 75 ? mb_substr($rawD, 0, 72) . '...' : $rawD);
                            ?></p>
        ...
        <aside class="dl-side-column">
            <section class="dl-widget">
                ...
                <div class="dl-quick-stats">
                    ...
                    <div><strong><?= count($myActiveTransactions) ?></strong><span>Kali Transaksi</span></div>
